  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="min-h-screen lg:pl-[248px]">
        ...
        {surface !== 'assistant' ? <WorkspaceSurface surface={surface} onUseAgent={(nextScenario, prompt) => { setScenario(nextScenario); setQuery(prompt); setDraft(''); setSurface('assistant'); setSent(true); }} /> : <><div className={`mx-auto flex w-full flex-col px-5 pb-[230px] md:px-10 ${sent ? 'max-w-[1120px] pt-8' : 'max-w-[980px] pt-[clamp(56px,9vh,96px)]'}`}>
          ...
          <div className="mt-9 grid gap-3 md:grid-cols-3">
            {quickActions.map(({ icon: Icon, title, hint, tone, scenario: nextScenario, prompt }) => (
              <button key={title} onClick={() => { setScenario(nextScenario); setQuery(prompt); setDraft(''); setSent(true); setApproved(false); }} className="group rounded-2xl border border-border bg-card p-4 text-left shadow-[0_12px_34px_rgba(39,55,45,.045)] transition-all hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-[0_16px_38px_rgba(39,55,45,.08)]">
        ...
        </div><PersistentComposer draft={draft} onDraftChange={setDraft} onSubmit={submit}/></>}
      </section>
    </main>
  );
}

function PersistentComposer({ draft, onDraftChange, onSubmit }: { draft: string; onDraftChange: (value: string) => void; onSubmit: () => void }) {
  return <div className="fixed bottom-4 left-0 right-0 z-40 px-5 lg:left-[248px] md:px-10"><div className="mx-auto max-w-[980px]"><div className="rounded-[22px] border border-border bg-white/95 p-2 shadow-[0_18px_70px_rgba(38,54,44,.14)] backdrop-blur-xl focus-within:border-primary/35 focus-within:ring-4 focus-within:ring-primary/5"><textarea value={draft} onChange={(e)=>onDraftChange(e.target.value)} onKeyDown={(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();onSubmit();}}} rows={3} placeholder="问制度、找文档，或交给我一项工作…" className="w-full resize-none bg-transparent px-3.5 pt-3 text-[15px] leading-6 outline-none placeholder:text-muted-foreground/60" aria-label="向工作助手提问"/><div className="flex items-center gap-2 px-1.5 pb-1.5"><Button variant="ghost" size="sm" className="rounded-lg text-muted-foreground"><Plus/> 添加上下文</Button><div className="ml-auto hidden items-center gap-2 text-[11px] text-muted-foreground sm:flex"><Command className="size-3"/> Enter 发送</div><Button onClick={onSubmit} disabled={!draft.trim()} size="icon" className="rounded-xl" aria-label="发送"><ArrowUp/></Button></div></div><div className="mt-2 flex items-center justify-center gap-4 text-[10px] text-muted-foreground/70"><span className="flex items-center gap-1"><ShieldCheck className="size-3"/> 权限感知</span><span className="flex items-center gap-1"><Clock3 className="size-3"/> 可回溯</span><span className="flex items-center gap-1"><CheckCircle2 className="size-3"/> 高风险操作需确认</span></div></div></div>;
}
...
function GeneralConversation({ query, onReset }: { query: string; onReset: () => void }) {
  ...
  return <div className="mx-auto max-w-[820px] animate-in fade-in duration-500"><div className="flex items-center justify-between"><div><div className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">Conversation</div><h1 className="mt-2 text-2xl font-semibold tracking-[-.035em]">新对话</h1></div><Button onClick={onReset} variant="outline" className="rounded-xl"><RotateCcw/> 重新体验</Button></div><section className="chat-thread mt-10 flex flex-col"><div className="chat-user ml-auto max-w-[76%] rounded-[20px] rounded-br-md bg-[#e3ebe5] px-4 py-3 text-sm leading-6">{query}</div><div className="chat-assistant mt-7 flex max-w-[88%] items-start gap-3"><div className="grid size-8 shrink-0 place-items-center rounded-full bg-primary text-white"><Sparkles className="size-3.5"/></div><div className="pt-1 text-sm leading-7 text-[#34443a]">{greeting ? <><p>你好，我在。</p><p className="mt-2 text-muted-foreground">你可以问我公司制度、找文档，或直接交给我一项工作。涉及提交、分派或变更数据时，我会先说明影响范围，再请你确认。</p></> : <><p>我收到了。为了避免误触发企业流程，还需要你补充一点信息：这是要查询制度、查看进度，还是让我代为执行？</p></>}</div></div></section></div>;
}
  ...
  return <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
    ...
    <div className="mt-7 grid gap-5 lg:grid-cols-[.72fr_1.28fr]">
      ...
      <section className="chat-thread flex flex-col">
        <div className="chat-user ml-auto max-w-[76%] rounded-[20px] rounded-br-md bg-[#e3ebe5] px-4 py-3 text-sm leading-6">{query}</div>
